...
it('should validate the credentials', async () => {
  ...
  expect(screen.getByText('Ange ett giltig personnummer.')).not.toHaveClass(
    'inputValidationVISIBLE'
  );
...
it('should submit the credentials if they are valid', async () => {
  ...
  const resultMock = jest.fn().mockReturnValue({
    data: {
      authenticate: {
        __typename: 'Token',
        token: 'validToken',
  ...
  await userEvent.type(
    screen.getByRole('textbox', { name: 'Personnummer' }),
    personalIdentityNumber
  );
  userEvent.click(screen.getByRole('button', { name: 'Fortsätt' }));
